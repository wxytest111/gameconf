var HE = {
	...
	getProp : function(obj,prop) {
		return obj[prop] || obj.getAttribute(prop);
	},
	...
	hasClass : function(obj,_class){
		return (' ' + obj.className + ' ').indexOf(' ' + _class + ' ')>-1;
	},
	addClass : function(obj,_class) {
		if(!HE.hasClass(obj,_class)) {
			obj.className +=  " " + _class;
		}	
	},
	removeClass : function(obj,_class) {
		if(HE.hasClass(obj,_class)){
			obj.className = HE.trim.both((" " + obj.className + " ").replace(" " + _class + " "," "));
		}	
	},
	...
	json : {
		...
		stringify : function(json){
		    switch (json.constructor) {
		        ...
		        case Array:
		            return '[' + json.map(gip_json_encode).join(',') + ']';
		        case Object:
		            var lxStr = [];
		            for (var property in json) 
		                if (typeof(json[property]) != "function") {
		                    lxStr.push(gip_json_encode(property) + ':' + gip_json_encode(json[property]));
		                }
		            return '{' + lxStr.join(',') + '}';
		        case Number:
		            if (isFinite(json)) {
